...
router.post('/order', authMiddleware, async (req, res) => {
  const { stock_symbol, stock_name, quantity, order_action, order_type, limit_price } = req.body;
  ...
  try {
    ...
    if (order_type === 'market') {
      await processOrder(userId, stock_symbol, stock_name, quantity, order_action, price, totalAmount);
      res.redirect('/dashboard');
    } else {
      await pool.query(
        'INSERT INTO pending_orders (user_id, stock_symbol, stock_name, quantity, order_action, order_type, limit_price, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
        [userId, stock_symbol, stock_name, quantity, order_action, order_type, limit_price, status]
      );
      res.redirect('/dashboard');
      checkPendingOrders(userId, stock_symbol, stock_name, quantity, order_action, limit_price);
    }
  } catch (err) {
    console.error(err);
...
async function processOrder(userId, stock_symbol, stock_name, quantity, order_action, price, totalAmount) {
  ...
  try {
    ...
    if (order_action === 'buy') {
      await client.query(
        'INSERT INTO holdings (user_id, stock_symbol, stock_name, quantity, invested) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (user_id, stock_symbol) DO UPDATE SET quantity = holdings.quantity + EXCLUDED.quantity, invested = holdings.invested + EXCLUDED.invested',
        [userId, stock_symbol, stock_name, quantity, totalAmount]
      );
      await client.query(
      ...
      await client.query(
        'INSERT INTO profits_losses (user_id, stock_symbol, stock_name, profit_loss) VALUES ($1, $2, $3, $4)',
        [userId, stock_symbol, stock_name, totalProfitLoss]
      );
    ...
    await client.query(
      'INSERT INTO transactions (user_id, stock_symbol, stock_name, quantity, order_action, order_type, limit_price, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [userId, stock_symbol, stock_name, quantity, order_action, 'market', price, 'success']
    );
